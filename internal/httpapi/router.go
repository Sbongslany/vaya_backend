...
import (
	...
	adminDep "github.com/yourorg/ehailing/backend/internal/admin/dependency"
	"github.com/yourorg/ehailing/backend/internal/auth/dependency"
	"github.com/yourorg/ehailing/backend/internal/auth/domain" // <-- CRITICAL IMPORT FOR RBAC
	chatDep "github.com/yourorg/ehailing/backend/internal/chat/dependency"
	"github.com/yourorg/ehailing/backend/internal/config"
	driverDep "github.com/yourorg/ehailing/backend/internal/driver/dependency"
	geofenceDep "github.com/yourorg/ehailing/backend/internal/geofence/dependency"
...
func NewRouter(
	...
	// ==========================================
	// ADMIN PROMOTIONS ROUTES (Finance + Super Admin)
	// ==========================================
	adminPromos := engine.Group("/api/v1/admin/promotions")
	adminPromos.Use(
		authContainer.Middleware.Authenticate(),
		authContainer.AdminMiddleware.RequireAdmin(),
		authContainer.Middleware.RequireRole(domain.RoleSuperAdmin, domain.RoleFinanceAdmin),
	)
	{
		adminPromos.POST("", promosContainer.AdminHandler.CreatePromotion)
		adminPromos.GET("", promosContainer.AdminHandler.ListPromotions)
	...
	// ==========================================
	// ADMIN WALLET ROUTES (Finance + Super Admin)
	// ==========================================
	adminWalletRoutes := engine.Group("/api/v1/admin/wallet")
	adminWalletRoutes.Use(
		authContainer.Middleware.Authenticate(),
		authContainer.AdminMiddleware.RequireAdmin(),
		authContainer.Middleware.RequireRole(domain.RoleSuperAdmin, domain.RoleFinanceAdmin),
	)
	...
	// Admin Support Routes (Support + Super Admin)
	adminSupportRoutes := engine.Group("/api/v1/admin/support")
	adminSupportRoutes.Use(
		authContainer.Middleware.Authenticate(),
		authContainer.AdminMiddleware.RequireAdmin(),
		authContainer.Middleware.RequireRole(domain.RoleSuperAdmin, domain.RoleSupportAdmin),
	)
	{
		adminSupportRoutes.POST("/tickets/:ticketId/resolve", supportContainer.Handler.ResolveTicket)
	}
	...
	// ==========================================
	// ADMIN DASHBOARD ROUTES (Split by Role)
	// ==========================================

	// 1. General Stats (All Admins)
	adminDashboardStats := engine.Group("/api/v1/admin/dashboard")
	adminDashboardStats.Use(
		authContainer.Middleware.Authenticate(),
		authContainer.AdminMiddleware.RequireAdmin(),
	)
	{
		adminDashboardStats.GET("/overview", adminContainer.Handler.GetPlatformOverview)
		adminDashboardStats.GET("/financials", adminContainer.Handler.GetFinancialSummary)
		adminDashboardStats.GET("/users", adminContainer.Handler.ListUsers)
		adminDashboardStats.PATCH("/users/:userId/status", adminContainer.Handler.UpdateUserStatus)
		adminDashboardStats.GET("/trips", adminContainer.Handler.ListAllTrips)
	}

	// 2. Live Operations (Ops + Safety + Super Admin)
	adminDashboardOps := engine.Group("/api/v1/admin/dashboard")
	adminDashboardOps.Use(
		authContainer.Middleware.Authenticate(),
		authContainer.AdminMiddleware.RequireAdmin(),
		authContainer.Middleware.RequireRole(domain.RoleSuperAdmin, domain.RoleOperationsAdmin, domain.RoleSafetyAdmin),
	)
	{
		adminDashboardOps.GET("/live-map", adminContainer.Handler.GetLiveMap)
		adminDashboardOps.POST("/trips/force-cancel", adminContainer.Handler.ForceCancelTrip)
		adminDashboardOps.POST("/trips/force-complete", adminContainer.Handler.ForceCompleteTrip)
	}

	// 3. Safety Alerts (Safety + Super Admin)
	adminDashboardSafety := engine.Group("/api/v1/admin/dashboard")
	adminDashboardSafety.Use(
		authContainer.Middleware.Authenticate(),
		authContainer.AdminMiddleware.RequireAdmin(),
		authContainer.Middleware.RequireRole(domain.RoleSuperAdmin, domain.RoleSafetyAdmin),
	)
	{
		adminDashboardSafety.GET("/safety/sos/active", adminContainer.Handler.GetActiveSOS)
	}

	// 4. Financial Controls (Finance + Super Admin)
	adminDashboardFinance := engine.Group("/api/v1/admin/dashboard")
	adminDashboardFinance.Use(
		authContainer.Middleware.Authenticate(),
		authContainer.AdminMiddleware.RequireAdmin(),
		authContainer.Middleware.RequireRole(domain.RoleSuperAdmin, domain.RoleFinanceAdmin),
	)
	{
		adminDashboardFinance.GET("/payouts/pending", adminContainer.Handler.GetPendingPayouts)
		adminDashboardFinance.POST("/payouts/approve", adminContainer.Handler.ApprovePayout)
		adminDashboardFinance.POST("/payouts/reject", adminContainer.Handler.RejectPayout)
	}

	// ==========================================
	// ADMIN KYC ROUTES (Ops + Super Admin)
	// ==========================================
	adminKYC := engine.Group("/api/v1/admin/kyc")
	adminKYC.Use(
		authContainer.Middleware.Authenticate(),
		authContainer.AdminMiddleware.RequireAdmin(),
		authContainer.Middleware.RequireRole(domain.RoleSuperAdmin, domain.RoleOperationsAdmin),
	)
	{
		adminKYC.GET("/pending", kycContainer.Handler.ListPendingKYC)
		adminKYC.GET("/drivers/:userId/documents", kycContainer.Handler.GetDriverDocuments)
	...
	// ==========================================
	// ADMIN GEOFENCE ROUTES (Ops + Super Admin)
	// ==========================================
	adminGeofence := engine.Group("/api/v1/admin/geofences")
	adminGeofence.Use(
		authContainer.Middleware.Authenticate(),
	...
	// ==========================================
	// ADMIN SETTINGS & VEHICLES ROUTES (Super Admin ONLY)
	// ==========================================
	adminSettings := engine.Group("/api/v1/admin/settings")
	adminSettings.Use(
		authContainer.Middleware.Authenticate(),
